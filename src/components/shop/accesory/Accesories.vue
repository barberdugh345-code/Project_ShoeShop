    ...
    <div class="category-page min-h-screen bg-white">
        ...
        <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            ...
            <div class="relative w-full aspect-square bg-gray-100 overflow-hidden">
                ...
                <span 
                v-if="item.featured" 
                class="absolute top-2 left-2 bg-black text-white text-[10px] font-bold uppercase px-2 py-0.5 rounded tracking-wider"
                >
                Featured
                </span>
                ...
                <span 
                v-if="!item.inStock" 
                class="absolute top-2 right-2 bg-red-600 text-white text-xs font-bold uppercase px-2 py-1 rounded"
                >
                Out of Stock
                </span>
            </div>
    ...
    return Array.from(new Set(Accesories.map(item => item.category).filter(Boolean)));
    });

    // Get unique brands
    const brands = computed(() => {
    return Array.from(new Set(Accesories.map(item => item.brand).filter(Boolean)));
    });

    // Filter & Sort Logic
    const filteredProducts = computed(() => {
    let result = [...Accesories];
    ...
    const addToCart = (product: typeof Accesories[0]) => {
