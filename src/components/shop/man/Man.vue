  ...
  <div class="category-page min-h-screen bg-white">
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
      ...
      <div class="flex flex-col sm:flex-row justify-between items-stretch sm:items-center border-b border-gray-200 pb-4 mb-6 gap-3 sm:gap-4">
        ...
        <div class="grid grid-cols-1 xs:grid-cols-3 sm:flex sm:flex-wrap items-center gap-2 sm:gap-4">
          <select
      ...
      <div v-if="filteredProducts.length === 0" class="text-center py-12 sm:py-16">
        <p class="text-gray-500 text-base sm:text-lg">No products found matching your filters.</p>
        <button
          @click="resetFilters"
...
const getImageUrl = (product: any): string => {
  ...
  const target = product.imageUrl || product.images;
...
const categories = computed(() => {
  return Array.from(new Set(Data_Type.map(item => item.category).filter(Boolean)));
});

const brands = computed(() => {
  return Array.from(new Set(Data_Type.map(item => item.brand).filter(Boolean)));
});
...
function handleAddToCart(product: Item_Type) {
...
function handleBuyNow(product: Item_Type) {
  if (!product.inStock) return;
