  ...
  <div class="category-page min-h-screen bg-white">
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
      ...
      <div v-if="filteredProducts.length === 0" class="text-center py-12 sm:py-16">
        <p class="text-gray-500 text-base sm:text-lg">No women's products found matching your criteria.</p>
        <button
          @click="resetFilters"
...
const categories = computed(() =>
  Array.from(new Set(Woman_Type.map(item => item.category).filter(Boolean)))
);

const brands = computed(() =>
  Array.from(new Set(Woman_Type.map(item => item.brand).filter(Boolean)))
);
...
function handleBuyNow(product: Shoe_woman | any) {
  if (!product.inStock) return;
