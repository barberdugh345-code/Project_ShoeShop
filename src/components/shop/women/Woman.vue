    ...
    <div class="category-page min-h-screen bg-white">
        ...
        <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            ...
            <div class="relative w-full aspect-square bg-gray-100 overflow-hidden">
                <!-- Displays the first image from item.images array or fallback -->
                <img 
    ...
    return Array.from(new Set(Woman_Type.map(item => item.category).filter(Boolean)));
    });

    // Get unique brands
    const brands = computed(() => {
    return Array.from(new Set(Woman_Type.map(item => item.brand).filter(Boolean)));
    });
    ...
    const addToCart = (product: typeof Woman_Type[0]) => {
